...
class UpdateStaticReferenceProcessing : FileBasedPostProcessing() {
  ...
  private fun applyProcessing(refElement: KtCallExpression, callee: CallableDescriptor, allFiles: List<KtFile>) {
    ...
    println("${callee.name.asString()} : ${referencedObject.hasDeclaration(callee.name.asString(), KtNamedFunction::class.java)}")
    println("${callee.name.asString()} : ${referencedObject.hasDeclaration(propertyNamesByAccessorName(callee.name).firstOrNull()?.identifier ?: "", KtProperty::class.java)}")
  }
